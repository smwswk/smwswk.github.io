...
def generate_webp(src, dst, size='600'):
    geom = f'{size}x{size}\\>'
    try:
        result = subprocess.run(
            ['convert', str(src), '-resize', geom, '-quality',
             '75' if size == '600' else '80', str(dst)],
            capture_output=True, timeout=60
        )
...
def main():
    ...
    for sha, fname in src_files.items():
        ...
        if need_thumb:
            ...
            thumb_webp_path = THUMBS_DIR / f"{stem}.webp"
            generate_webp(src_path, thumb_webp_path, '600')
        else:
            skip_count += 1
        ...
        if need_web:
            ...
            web_webp_path = WEB_DIR / f"{stem}.webp"
            generate_webp(src_path, web_webp_path, '1920')
