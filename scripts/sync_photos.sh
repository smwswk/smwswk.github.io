...
for f in "$SRC"/*.{jpg,JPG,jpeg,JPEG,png,PNG,tif,TIF,tiff,TIFF} 2>/dev/null; do
    [ -e "$f" ] || continue
    fname=$(basename "$f")
...
# 生成缩略图（跳过已存在的）
for f in "$SRC"/*.{jpg,JPG,jpeg,JPEG} 2>/dev/null; do
    [ -e "$f" ] || continue
    fname=$(basename "$f")
    out="$THUMBS/$fname"
...
photos_json='['
first=true
for f in $(ls -t "$SRC"/*.{jpg,JPG,jpeg,JPEG} 2>/dev/null); do
    [ -e "$f" ] || continue
    fname=$(basename "$f")
    thumb="$THUMBS/$fname"
    full="$STATIC/$fname"
...
echo "  ✅ data/photos.json 已更新 ($(ls $SRC/*.{jpg,JPG,jpeg,JPEG} 2>/dev/null | wc -l | tr -d ' ') 张照片)"
